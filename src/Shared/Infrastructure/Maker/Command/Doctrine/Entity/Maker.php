 ...
 */
final class Maker extends AbstractMaker implements InputAwareMakerInterface
{
    private readonly Generator $generator;
    private readonly SyliusEntityClassGenerator $entityClassGenerator;
    ...
    public function __construct(
        ...
        $this->entityClassGenerator = new SyliusEntityClassGenerator($this->generator, $this->doctrineHelper, $this->fileManager);
    }

    public static function getCommandName(): string
    {
        return 'make:infrastructure:persistence:entity';
    }
    ...
    public function interact(InputInterface $input, ConsoleStyle $io, Command $command): void
    {
        ...
        if ($input->getOption('regenerate')) {
            ...
            $classOrNamespace = $io->ask('Enter a class or namespace to regenerate', $this->getEntityNamespace(), fn (?string $value = null): string => \Symfony\Bundle\MakerBundle\Validator::notBlank($value));
        ...
        if (
            !$input->getOption('sylius-crud') &&
            class_exists(SyliusCrudRoutes::class) &&
            !class_exists($this->generator->createClassNameDetails(
                $input->getArgument('name'),
                $configuration->getEntityPath(),
            )->getFullName())
        ) {
            $description = $command->getDefinition()->getOption('sylius-crud')->getDescription();
            $question = new ConfirmationQuestion($description, false);
            $isApiResource = $io->askQuestion($question);

            $input->setOption('sylius-crud', $isApiResource);
        }
    }
    ...
    private function createEntityClassQuestion(string $questionText): Question
    {
        ...
        $question->setValidator(fn (?string $value = null): string => \Symfony\Bundle\MakerBundle\Validator::notBlank($value));
